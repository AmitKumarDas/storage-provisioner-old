...
// secretParamsMap provides a mapping of secret keys
...
const (
	// DDI Provisioner Parameters prefixed with provisionerParameterPrefix
	// are not passed through to the driver on CreateVolumeRequest calls.
	// Instead they are intended to used by the storage-provisioner and
	// maybe used to populate fields in subsequent CSI calls or Kubernetes
	// API objects.
	provisionerParameterPrefix = "provisioner.ddi.openebs.io/"

	prefixedFsTypeKey = provisionerParameterPrefix + "fstype"

	prefixedProvisionerSecretNameKey = provisionerParameterPrefix +
		"provisioner-secret-name"

	prefixedProvisionerSecretNamespaceKey = provisionerParameterPrefix +
		"provisioner-secret-namespace"

	prefixedControllerPublishSecretNameKey = provisionerParameterPrefix +
		"controller-publish-secret-name"

	prefixedControllerPublishSecretNamespaceKey = provisionerParameterPrefix +
		"controller-publish-secret-namespace"

	prefixedNodeStageSecretNameKey = provisionerParameterPrefix +
		"node-stage-secret-name"

	prefixedNodeStageSecretNamespaceKey = provisionerParameterPrefix +
		"node-stage-secret-namespace"

	prefixedNodePublishSecretNameKey = provisionerParameterPrefix +
		"node-publish-secret-name"

	prefixedNodePublishSecretNamespaceKey = provisionerParameterPrefix +
		"node-publish-secret-namespace"

	prefixedControllerExpandSecretNameKey = provisionerParameterPrefix +
		"controller-expand-secret-name"

	prefixedControllerExpandSecretNamespaceKey = provisionerParameterPrefix +
		"controller-expand-secret-namespace"

	// [Deprecated] DDI Parameters that are put into fields but
	// NOT stripped from the parameters passed to CreateVolume
	//provisionerSecretNameKey      = "ddiProvisionerSecretName"
	//provisionerSecretNamespaceKey = "ddiProvisionerSecretNamespace"

	//controllerPublishSecretNameKey      = "ddiControllerPublishSecretName"
	//controllerPublishSecretNamespaceKey = "ddiControllerPublishSecretNamespace"

	//nodeStageSecretNameKey      = "ddiNodeStageSecretName"
	//nodeStageSecretNamespaceKey = "ddiNodeStageSecretNamespace"

	//nodePublishSecretNameKey      = "ddiNodePublishSecretName"
	//nodePublishSecretNamespaceKey = "ddiNodePublishSecretNamespace"
...
// requiredCapabilities provides a set of extra capabilities required
// for special/optional features provided by a plugin
type requiredCapabilities struct {
	snapshot bool
	clone    bool
}

type ddiProvisioner struct {
...
// TODO (@amitkumardas) These will eventually depend on Metac
// controller library
var _ controller.Provisioner = &ddiProvisioner{}
var _ controller.BlockProvisioner = &ddiProvisioner{}
var _ controller.ProvisionerExt = &ddiProvisioner{}
...
var (
	...
	provisionerIDKey = "ddi.openebs.io/provisionerIdentity"
)

// Connect opens an insecure gRPC connection to CSI driver
...
// Probe invokes probe of CSI driver and waits until the driver is
// ready
...
// GetDriverCapabilities returns the supported capabilities of CSI driver
func GetDriverCapabilities(
	conn *grpc.ClientConn,
	timeout time.Duration,
) (connection.PluginCapabilitySet, connection.ControllerCapabilitySet, error) {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
...
// NewDDIProvisioner creates new DDI provisioner
func NewDDIProvisioner(
	client kubernetes.Interface,
...
// This function get called before any attempt to communicate with
// the CSI driver. Before initiating Create/Delete API calls provisioner
// checks if following capabilities are supported:
//
//	- PluginControllerService,
//	- ControllerCreateVolume
func (p *ddiProvisioner) checkDriverCapabilities(rc *requiredCapabilities) error {
	provisionErrTitle := "CSI driver does not support dynamic provisioning"
	snapErrTitle := "CSI driver does not support snapshot restore"
	cloneErrTitle := "CSI driver does not support clone operations"

	if !p.pluginCapabilities[csi.PluginCapability_Service_CONTROLLER_SERVICE] {
		return errors.Errorf(
			"%s: plugin CONTROLLER_SERVICE capability is not reported",
			provisionErrTitle,
		)
	}
	...
	if !p.controllerCapabilities[csi.ControllerServiceCapability_RPC_CREATE_DELETE_VOLUME] {
		return errors.Errorf(
			"%s: controller CREATE_DELETE_VOLUME capability is not reported",
			provisionErrTitle,
		)
	}
	...
	if rc.snapshot {
		...
		if !p.controllerCapabilities[csi.ControllerServiceCapability_RPC_CREATE_DELETE_SNAPSHOT] {
			return errors.Errorf(
				"%s: controller CREATE_DELETE_SNAPSHOT capability is not reported",
				snapErrTitle,
			)
		}
	}
	if rc.clone {
		...
		if !p.controllerCapabilities[csi.ControllerServiceCapability_RPC_CLONE_VOLUME] {
			return errors.Errorf(
				"%s: controller CLONE_VOLUME capability is not reported",
				cloneErrTitle,
			)
		}
	}
...
func makeVolumeName(prefix, pvcUID string, volumeNameUUIDLength int) (string, error) {
	...
	return fmt.Sprintf(
		"%s-%s",
		prefix,
		strings.Replace(string(pvcUID), "-", "", -1)[0:volumeNameUUIDLength],
	), nil
...
func (p *ddiProvisioner) Provision(
	options controller.ProvisionOptions,
...
func (p *ddiProvisioner) ProvisionExt(
	options controller.ProvisionOptions,
...
func (p *ddiProvisioner) supportsTopology() bool {
...
func removePrefixedParameters(param map[string]string) (map[string]string, error) {
	...
	for k, v := range param {
		if strings.HasPrefix(k, provisionerParameterPrefix) {
			...
			default:
				return map[string]string{}, fmt.Errorf("found unknown parameter key \"%s\" with reserved namespace %s", k, provisionerParameterPrefix)
			}
		} else {
...
func (p *ddiProvisioner) getVolumeContentSource(options controller.ProvisionOptions) (*csi.VolumeContentSource, error) {
	switch options.PVC.Spec.DataSource.Kind {
	case snapshotKind:
...
func (p *ddiProvisioner) getPVCSource(options controller.ProvisionOptions) (*csi.VolumeContentSource, error) {
...
func (p *ddiProvisioner) getSnapshotSource(options controller.ProvisionOptions) (*csi.VolumeContentSource, error) {
...
func (p *ddiProvisioner) Delete(volume *v1.PersistentVolume) error {
	if volume == nil {
...
func (p *ddiProvisioner) SupportsBlock() bool {
...
func (p *ddiProvisioner) volumeIdToHandle(id string) string {
	return id
}

func (p *ddiProvisioner) volumeHandleToId(handle string) string {
	return handle
}
...
func verifyAndGetSecretNameAndNamespaceTemplate(secret secretParamsMap, storageClassParams map[string]string) (nameTemplate, namespaceTemplate string, err error) {
	...
	if numName > 1 || numNamespace > 1 {
		...
		return "", "", fmt.Errorf("%s secrets specified in parameters with both \"csi\" and \"%s\" keys", secret.name, provisionerParameterPrefix)
